...
export class StatusRenderer {
	...
	renderStaged(staged) {
		...
		if (filtered.length) {
			...
			html =
				'<li class="file-item untracked"><span class="dot"></span><span class="name">No staged changes</span><span class="badge">✓</span></li>';
		}
	...
	renderDirty(status) {
		...
		if (Array.isArray(status.modified) && status.modified.length) {
			html += status.modified
				.map((n) => this.renderFileItem(this.entryPath(n), "modified", "M"))
				.join("");
		}

		if (Array.isArray(status.deleted) && status.deleted.length) {
			html += status.deleted
				.map((n) => this.renderFileItem(this.entryPath(n), "deleted", "D"))
				.join("");
		}

		if (Array.isArray(status.untracked) && status.untracked.length) {
			html += status.untracked
				.map((n) => this.renderFileItem(this.entryPath(n), "untracked", "?"))
				.join("");
		}

		if (!html) {
			html =
				'<li class="file-item untracked"><span class="dot"></span><span class="name">Clean working tree</span><span class="badge">✓</span></li>';
		}
			...
			.map((item) => {
				...
				if (item && typeof item === "object") {
					const path =
						...
						"";
					const code =
						item.status || item.code || item.x || item.y || item.xy || null;
...
export class ConfigRenderer {
	...
	parseIni(str) {
		...
		for (const raw of lines) {
			...
			const kv =
				line.match(/^([^=\s]+)\s*=\s*(.*)$/) || line.match(/^([^\s]+)\s+(.*)$/);
			if (kv) {
				const key = kv[1];
				const val = kv[2];

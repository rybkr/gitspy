...
class GraphVisualization {
    ...
    setupSimulation(nodes, links) {
        ...
        this.simulation = d3.forceSimulation(nodes)
            .force('link', d3.forceLink(links).id(d => d.id).distance(d => d.isBranchLink ? 120 : 80))
    ...
    renderLinks(links) {
        this.linkSelection = this.mainGroup.append('g')
            .selectAll('path')
            .data(links)
            .enter()
            .append('path')
    ...
    renderNodes(nodes) {
        this.nodeSelection = this.mainGroup.append('g')
            .selectAll('g')
            .data(nodes)
            .enter()
            .append('g')
    ...
    renderBranches(branchNodes) {
        this.branchSelection = this.mainGroup.append('g')
            .selectAll('g')
            .data(branchNodes)
            .enter()
            .append('g')
            ...
            .on('click', (event, d) => {
                event.stopPropagation();
                this.onNodeClick({ hash: d.name, message: 'Branch', branches: [d.name] }, event);
            });
        ...
        this.branchSelection.append('rect')
            .attr('x', d => -d._w / 2)
            .attr('y', d => -d._h / 2)
            .attr('rx', 9)
            .attr('ry', 9)
        ...
        this.branchSelection.append('text')
    ...
    onTick() {
        ...
        if (this.linkSelection) {
            this.linkSelection.attr('d', d => {
                const sx = (typeof d.source === 'object' ? d.source.x : this.idToNode.get(d.source)?.x) || 0;
                const sy = (typeof d.source === 'object' ? d.source.y : this.idToNode.get(d.source)?.y) || 0;
                const tx = (typeof d.target === 'object' ? d.target.x : this.idToNode.get(d.target)?.x) || 0;
                const ty = (typeof d.target === 'object' ? d.target.y : this.idToNode.get(d.target)?.y) || 0;
    ...
    applyTimeFilter(timeRange) {
        ...
        if (this.linkSelection) {
            this.linkSelection.style('display', (d) => {
                if (d.isBranchLink) {
                    const targetNode = typeof d.target === 'object' ? d.target : this.idToNode.get(d.target);
                    const ts = targetNode ? nodeHasTimestamp(targetNode) : null;
                    return ts == null ? 'none' : (ts >= start && ts <= end ? null : 'none');
                } else {

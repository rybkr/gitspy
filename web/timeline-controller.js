...
class TimelineController {
    ...
    positionHandles() {
        ...
        if (!this.elements.startHandle || !this.elements.endHandle || !this.elements.fill) {
            return;
        }
        ...
        this.elements.startHandle.setAttribute('aria-valuenow', String(percentage(this.startTs)));
        this.elements.endHandle.setAttribute('aria-valuenow', String(percentage(this.endTs)));
    }
    ...
    setupDragHandlers() {
        ...
        if (this.elements.startHandle) {
            this.elements.startHandle.addEventListener('mousedown', onDown('start', this.elements.startHandle));
            this.elements.startHandle.addEventListener('click', () => this.elements.startHandle.focus());
        }

        if (this.elements.endHandle) {
            this.elements.endHandle.addEventListener('mousedown', onDown('end', this.elements.endHandle));
            this.elements.endHandle.addEventListener('click', () => this.elements.endHandle.focus());
        }
    }
    ...
    setupKeyboardControls() {
        const handleKey = (e, which) => {
            ...
            if (which === 'start') {
                ...
                if (list[i] < current && direction > 0) i = Math.min(i + 1, list.length - 1);
        ...
        if (this.elements.startHandle) {
            this.elements.startHandle.addEventListener('keydown', (e) => handleKey(e, 'start'));
        }
        if (this.elements.endHandle) {
            this.elements.endHandle.addEventListener('keydown', (e) => handleKey(e, 'end'));
        }
    }
